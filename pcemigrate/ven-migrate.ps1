...
# Declare Variables
param([switch]$use_configfile, [string]$pce=$null, [string]$activation_code=$null, [string]$port="443", [string]$migrate_type=$null, [string]$api_version="v25", [string]$profile_id=$null, [string]$vendir="C:\Program Files\Illumio")
...
function Usage() {
    Write-Host
    Write-Host "Usage: $scriptname -use_configfile -pce PCE [-port PORT] -activation_code ACTIVATION_CODE -migrate_type [activate|pair] [ -api_version
...
if ( $rc = Check-File-Exists $wkldfile -eq 1 ) {
    exit 1
}
