...
exec_workloader() {
  ...
  PCE=$5
  echo 
  echo ". executing $WORKLOADER_CMD"
  echo
  IFS=
  read -p ". press enter to continue, s to skip, or q to quit? " ANS

  if [ -z $ANS ]; then
    touch $OUTFILE
    if [ "$WCMD" = "svc-import" ]; then
      echo ". $WORKLOADER $WCMD $OUTFILE --update-pce --no-prompt --update-on-name"
      $WORKLOADER $WCMD $OUTFILE --update-pce --no-prompt --update-on-name
    elif [ "$WCMD" = "wkld-import" ]; then
      echo ". $WORKLOADER $WCMD $OUTFILE --umwl --update-pce --no-prompt"
      $WORKLOADER $WCMD $OUTFILE --umwl --update-pce --no-prompt
    elif [ "$WCMD" = "rule-import" ]; then
      echo ". $WORKLOADER $WCMD $OUTFILE --continue-on-error --update-pce --no-prompt"
      $WORKLOADER $WCMD $OUTFILE --continue-on-error --update-pce --no-prompt
    else
      echo ". $WORKLOADER $WCMD $OUTFILE --update-pce --no-prompt"
      $WORKLOADER $WCMD $OUTFILE --update-pce --no-prompt --pce $TO_PCE
    fi

  elif [ $ANS = "s" ]; then
    echo ". skipping $WORKLOADER_CMD"
...
export_objects() {
  ...
  [[ -r $WKLD_LABEL_ONLY ]] && cp -p $WKLD_LABEL_ONLY $WKLD_LABEL_ONLY.$DTE
  echo
...
import_objects() {
  ...
  for WCMD in $(echo $WORKLOADER_CMDS); do
    ...
    if [ "$WCMD" = "svc-import" ]; then
       WORKLOADER="$BASEDIR/workloader.910"
       WORKLOADER="$BASEDIR/workloader"
       WORKLOADER_CMD="$WORKLOADER $WCMD $OUTFILE --update-pce --no-prompt --update-on-name"
       exec_workloader "$WORKLOADER_CMD" "$WORKLOADER" "$WCMD" "$OUTFILE" 
    fi

    if [ "$WCMD" = "label-dimension-import" ]; then
      echo ". label dimension is only supported in pce 22.5.32 or later"
    fi

    if [ "$WCMD" = "adgroup-import" ] || [ "$WCMD" = "label-dimension-import" ] || [ "$WCMD" = "label-import" ] || [ "$WCMD" = "ipl-import" ] || [ "$WCMD" = "labelgroup-import" ] || [ "$WCMD" = "eb-import" ] ||  [ "$WCMD" = "ruleset-import" ] || [ "$WCMD" = "rule-import" ]; then
      WORKLOADER_CMD="$WORKLOADER $WCMD $OUTFILE --update-pce --no-prompt"
      exec_workloader "$WORKLOADER_CMD" "$WORKLOADER" "$WCMD" "$OUTFILE"
    fi
